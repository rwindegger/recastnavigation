								  ...
								  dtTileCachePolyMesh& mesh)
{
	...
	mesh.areas = (dtAreaId*)alloc->alloc(sizeof(dtAreaId)*maxTris);
	if (!mesh.areas)
		return DT_FAILURE | DT_OUT_OF_MEMORY;

	mesh.flags = (dtPolyFlags*)alloc->alloc(sizeof(dtPolyFlags)*maxTris);
	if (!mesh.flags)
		return DT_FAILURE | DT_OUT_OF_MEMORY;

	// Just allocate and clean the mesh flags array. The user is resposible for filling it.
	memset(mesh.flags, 0, sizeof(dtPolyFlags) * maxTris);
	...
	memset(mesh.areas, 0, sizeof(dtAreaId)*maxTris);
							   ...
							   unsigned char** outData, int* outDataSize)
{
	...
	const int bufferSize = gridSize*2*sizeof(char) + gridSize*sizeof(dtAreaId);
	unsigned char* buffer = (unsigned char*)dtAlloc(bufferSize, DT_ALLOC_TEMP);
	if (!buffer)
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	memcpy(buffer, heights, gridSize);
	memcpy(buffer+gridSize, areas, gridSize*sizeof(dtAreaId));
	memcpy(buffer+(gridSize + gridSize*sizeof(dtAreaId)), cons, gridSize);
									...
									dtTileCacheLayer** layerOut)
{
	...
	*layerOut = 0;
	...
	const int bufferSize = layerSize + headerSize + gridSize*3 + gridSize*sizeof(dtAreaId);
	...
	layer->header = header;
	layer->heights = grids;
	layer->areas = (dtAreaId*)(grids + gridSize);
	layer->cons = grids + gridSize+gridSize*sizeof(dtAreaId);
	layer->regs = grids + gridSize*2+gridSize*sizeof(dtAreaId);
