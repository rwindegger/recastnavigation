...
{
	if (ImGui::Selectable("Pathfind Follow", m_toolMode == TOOLMODE_PATHFIND_FOLLOW))
	{
		m_toolMode = TOOLMODE_PATHFIND_FOLLOW;
		recalc();
	}
	if (ImGui::Selectable("Pathfind Straight", m_toolMode == TOOLMODE_PATHFIND_STRAIGHT))
	{
	...
	if (m_toolMode == TOOLMODE_PATHFIND_STRAIGHT)
	{
		...
		ImGui::Separator();
		if (ImGui::Selectable("None", m_straightPathOptions == 0))
		{
			m_straightPathOptions = 0;
			recalc();
		}
		if (ImGui::Selectable("Area", m_straightPathOptions == DT_STRAIGHTPATH_AREA_CROSSINGS))
		{
			m_straightPathOptions = DT_STRAIGHTPATH_AREA_CROSSINGS;
			recalc();
		}
		if (ImGui::Selectable("All", m_straightPathOptions == DT_STRAIGHTPATH_ALL_CROSSINGS))
		{
			m_straightPathOptions = DT_STRAIGHTPATH_ALL_CROSSINGS;
			recalc();
		}
	}
	if (ImGui::Selectable("Pathfind Sliced", m_toolMode == TOOLMODE_PATHFIND_SLICED))
	{
	...
	if (ImGui::Selectable("Distance to Wall", m_toolMode == TOOLMODE_DISTANCE_TO_WALL))
	{
	...
	if (ImGui::Selectable("Raycast", m_toolMode == TOOLMODE_RAYCAST))
	{
	...
	if (ImGui::Selectable("Find Polys in Circle", m_toolMode == TOOLMODE_FIND_POLYS_IN_CIRCLE))
	{
		m_toolMode = TOOLMODE_FIND_POLYS_IN_CIRCLE;
		recalc();
	}
	if (ImGui::Selectable("Find Polys in Shape", m_toolMode == TOOLMODE_FIND_POLYS_IN_SHAPE))
	{
	...
	if (ImGui::Selectable("Find Local Neighbourhood", m_toolMode == TOOLMODE_FIND_LOCAL_NEIGHBOURHOOD))
	{
	...
	{
		navAreaMask includeMask = m_filter.getIncludeFlags();

		if (ImGui::CheckboxFlags("Walk", &includeMask, AREAFLAGS_WALK))
		{
			m_filter.setIncludeFlags(includeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Swim", &includeMask, AREAFLAGS_SWIM))
		{
			m_filter.setIncludeFlags(includeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Door", &includeMask, AREAFLAGS_DOOR))
		{
			m_filter.setIncludeFlags(includeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Crouch", &includeMask, AREAFLAGS_CROUCH))
		{
			m_filter.setIncludeFlags(includeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Jump", &includeMask, AREAFLAGS_JUMP))
		{
			m_filter.setIncludeFlags(includeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Disabled", &includeMask, AREAFLAGS_DISABLED))
		{
			m_filter.setIncludeFlags(includeMask);
			recalc();
		}
	...
	{
		navAreaMask excludeMask = m_filter.getExcludeFlags();
		if (ImGui::CheckboxFlags("Walk", &excludeMask, AREAFLAGS_WALK))
		{
			m_filter.setExcludeFlags(excludeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Swim", &excludeMask, AREAFLAGS_SWIM))
		{
			m_filter.setExcludeFlags(excludeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Door", &excludeMask, AREAFLAGS_DOOR))
		{
			m_filter.setExcludeFlags(excludeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Crouch", &excludeMask, AREAFLAGS_CROUCH))
		{
			m_filter.setExcludeFlags(excludeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Jump", &excludeMask, AREAFLAGS_JUMP))
		{
			m_filter.setExcludeFlags(excludeMask);
			recalc();
		}
		if (ImGui::CheckboxFlags("Disabled", &excludeMask, AREAFLAGS_DISABLED))
		{
			m_filter.setExcludeFlags(excludeMask);
			recalc();
		}
