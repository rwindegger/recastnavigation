...
void CrowdToolState::init(class Sample* sample)
{
	...
	if (nav && crowd && (m_nav != nav || m_crowd != crowd))
	{
		...
		crowd->getEditableFilter(0)->setExcludeFlags(AREAFLAGS_DISABLED);
...
/*
		...
		ImGui::PlotLines("Total", [](void* data, int idx) {
			ValueHistory* vh = (ValueHistory*)data;
			return vh->getSample(idx);
		}, &m_crowdSampleCount, 256, 0, "", 0.0f, 2000.0f, ImVec2(500, 50));
...
void CrowdTool::handleMenu()
{
	...
	if (ImGui::CollapsingHeader("Options"))
	{
		...
		if (ImGui::SliderFloat("Avoidance Quality", &params->m_obstacleAvoidanceType, 0.0f, 3.0f))
		{
			m_state->updateAgentParams();
		}
