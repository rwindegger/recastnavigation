...
void Sample_SoloMesh::handleTools()
{
	...
	if (ImGui::Selectable("Create Off-Mesh Connections", type == TOOL_OFFMESH_CONNECTION))
	{
...
bool Sample_SoloMesh::handleBuild()
{
	...
	for (int i = 0; i < m_geom->getConvexVolumeCount(); ++i)
		rcMarkConvexPolyArea(m_ctx, vols[i].verts, vols[i].nverts, vols[i].hmin, vols[i].hmax, (unsigned char)vols[i].areaMask, *m_chf);
