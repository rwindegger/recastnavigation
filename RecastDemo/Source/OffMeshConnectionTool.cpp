...
void OffMeshConnectionTool::handleClick(const float* /*s*/, const float* p, bool shift)
{
	...
	else
	{
		...
		{
			geom->addOffMeshConnection(m_hitPos, p, m_sample->getAgentRadius(), m_bidir ? 1 : 0, AREAFLAGS_JUMP);
			m_hitPosSet = false;
		}
	}
...
void OffMeshConnectionTool::handleRenderOverlay(double* proj, double* model, int* view)
{
	...
	if (!m_hitPosSet)
	{
		ImGui::TextColored(ImVec4(1.0, 1.0, 1.0, 0.5), "LMB: Create new connection.  SHIFT+LMB: Delete existing connection, click close to start or end point.");
	}
	else
	{
		ImGui::TextColored(ImVec4(1.0, 1.0, 1.0, 0.5), "LMB: Set connection end point and finish.");
	}
}
