						   ...
						   bool& isBorderVertex)
{
	...
	unsigned short regionIds[4] = {0,0,0,0};
	rcAreaId areaIds[4] = {0,0,0,0};
	//unsigned int regs[4] = {0,0,0,0};
	
	// Combine region and area codes in order to prevent
	// border vertices which are in between two areas to be removed.
	//regs[0] = chf.spans[i].reg | (chf.areas[i] << 16);
	regionIds[0] = chf.spans[i].reg;
	areaIds[0] = chf.areas[i];
	...
	if (rcGetCon(s, dir) != RC_NOT_CONNECTED)
	{
		...
		regionIds[1] = chf.spans[ai].reg;
		areaIds[1] = chf.areas[ai];
		if (rcGetCon(as, dirp) != RC_NOT_CONNECTED)
		{
			...
			regionIds[2] = chf.spans[ai2].reg;
			areaIds[2] = chf.areas[ai2];
		}
	}
	if (rcGetCon(s, dirp) != RC_NOT_CONNECTED)
	{
		...
		regionIds[3] = chf.spans[ai].reg;
		areaIds[3] = chf.areas[ai];
	...
	for (int j = 0; j < 4; ++j)
	{
		...
		const bool twoSameExts = (regionIds[a] & regionIds[b] & RC_BORDER_REG) != 0 && (regionIds[a] == regionIds[b] && areaIds[a] == areaIds[b]);

		//const bool twoInts = ((regs[c] | regs[d]) & RC_BORDER_REG) == 0;
		const bool twoInts = ((regionIds[c] | regionIds[d]) & RC_BORDER_REG) == 0;

		//const bool intsSameArea = (regs[c]>>16) == (regs[d]>>16);
		const bool intsSameArea = (areaIds[c] == areaIds[d]);

		//const bool noZeros = regs[a] != 0 && regs[b] != 0 && regs[c] != 0 && regs[d] != 0;
		const bool noZeros = regionIds[a] != 0 && areaIds[a] != 0 && regionIds[b] != 0 && areaIds[b] != 0 && regionIds[c] != 0 && areaIds[c] != 0 && regionIds[d] != 0 && areaIds[d] != 0;
						...
						unsigned char* flags, rcIntArray& points)
{
	...
	const rcAreaId area = chf.areas[i];
